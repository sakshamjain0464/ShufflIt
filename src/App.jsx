  ...
  let searchImages = async (event, pageNo) => {
    ...
    try {
      let json = await fetch(
        `https://api.unsplash.com/search/photos?page=${pageNo}&query=${search.trim()}&client_id=${key}&per_page=${pageSize}`
      );
    ...
    <>
      <div className="head">
        ...
        <div className="images-container">
          ...
          {attributes.query && (
            <h1>Images for &quot;{attributes.query}&quot;</h1>
          )}
          {attributes.searched && !attributes.found && <Loading />}
          {attributes.query && (
            <p>
              {attributes.total} Images found for &quot;{attributes.query}&quot;
            </p>
          )}
          {attributes.found && (
            <div className="images">
              {data.results.map((image) => (
                <Card
                  ...
                  imgUrl={image.links.download}
                />
              ))}
            </div>
          )}
          {attributes.found && attributes.total != [] && (
            <div className="page-buttons">
              <button className="previous" onClick={goPrevious}>
                <i className="fa-solid fa-chevron-left"></i> Previous
              </button>
              <p>
                {attributes.currentPage} of {attributes.pages}
              </p>
